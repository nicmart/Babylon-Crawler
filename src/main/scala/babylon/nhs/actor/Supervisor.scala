...
import babylon.nhs.actor.Crawler.StartCrawling
...
/**
  ...
  */
class Supervisor(writer: Writer[PageList]) extends Actor with ActorLogging {
    import Supervisor._

    val crawler: ActorRef = context.actorOf(Props(new Crawler), "crawler")
